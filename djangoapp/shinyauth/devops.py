...
def generate_deployment(app):
    ...
    with open(yaml_template) as f:
        ...
        template_lines = [
            line.replace("$APP_SLUG", app_slug) for line in template_lines
        ]
        template_lines = [
    ...
    with open(sh_template) as f:
        ...
        template_lines = [
            line.replace("$GIT_REPO", git_repo) for line in template_lines
        ]
        template_lines = [
            line.replace("$GIT_BRANCH", git_branch) for line in template_lines
        ]
        template_lines = [
            line.replace("$CLOUDBUILD_CONNECTION", cloudbuild_connection)
            for line in template_lines
        ]
        template_lines = [
            line.replace("$REPO_NAME", repo_name) for line in template_lines
        ]
        template_lines = [
            line.replace("$GCP_PROJECT_ID", GCP_PROJECT_ID) for line in template_lines
        ]
    ...
    with open(sh_delete_template) as f:
        ...
        template_lines = [
            line.replace("$REGION", GCP_PROJECT_ID) for line in template_lines
        ]
    ...
    with open(k8s_template) as f:
        ...
        template_lines = [
            line.replace("$APP_IMAGE", app_image) for line in template_lines
        ]
        template_lines = [
            line.replace("$APP_PORT", app_port) for line in template_lines
        ]
        ...
        template_lines = [
            line.replace("$HOSTNAME", HOSTNAME) for line in template_lines
        ]
    ...
    print(
        f"Done creating cloudbuild YAML, gcloud scripts and k8s YAML files for {app}."
    )
